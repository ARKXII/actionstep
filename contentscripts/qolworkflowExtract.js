function extractTreeToJSON(currentURL, environment) {
    ...
    <div style="border: 1px solid #ccc; padding: 10px; margin-top: 10px; background: #f9f9f9; border-radius: 4px;">
      <div style="margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center;">
        ...
        <div>
          ...
          <button id="copy-json-btn" style="padding: 4px 10px; background: #2196F3; color: white; border: none; border-radius: 3px; cursor: pointer; font-size: 13px;">
            Copy JSON
          </button>
        </div>
      </div>
      <textarea id="tree-json-textarea" style="width:100%; height:300px; font-family: monospace; font-size: 12px; padding: 8px; border: 1px solid #ddd; border-radius: 4px; background: white;" readonly>${jsonOutput}</textarea>
      <div style="margin-top: 8px; font-size: 12px; color: #666;">
        Total nodes: ${treeData.length} root node(s) | Depth: Check hierarchy in JSON
      </div>
    </div>
  `;
  ...
  if (insertTarget) {
    ...
    const existingContainer = document.getElementById("tree-json-container");
